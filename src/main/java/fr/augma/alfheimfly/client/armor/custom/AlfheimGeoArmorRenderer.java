...
public class AlfheimGeoArmorRenderer<T extends ItemArmor & IAnimatable> extends GeoArmorRenderer<T> {
	
	private static Map<Class<? extends ItemArmor>, GeoArmorRenderer> renderers = new ConcurrentHashMap<>();

	public AlfheimGeoArmorRenderer(AnimatedGeoModel<T> modelProvider) {
		super(modelProvider);
	...
	public void render(float partialTicks) {
		...
		if (isSneak)
		{
			...
			try
			{
				headBone.setPositionY(headBone.getPositionY() - 1.15F);
	...
	@Override
	public AlfheimGeoArmorRenderer<T> applySlot(EntityEquipmentSlot slot)
	{
